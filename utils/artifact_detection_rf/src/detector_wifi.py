...
class RSSIPublisher:
    def __init__(self):
        ...
        # self.sub_channels = rospy.Subscriber("active_channels", Int8MultiArray, self.channel_clbk, queue_size=10)
    ...
    def check_channels(self):
        if rospy.has_param('active_channels'):
            ...
            if len(active_channels) > 0:
                print '[WiFi Det] Active channels:', active_channels
                for channel in active_channels:
                    rospy.loginfo("[WiFi Det] Listening to channel %i", channel)
                    self.get_rssi(channel)
            else: 
                rospy.loginfo('No WiFi hotspots in range...')
    ...
    def set_channel(self, channel):
        ...
        if result == 0:
            ...
            rospy.loginfo("[WiFi Det] FAILED TO change to channel %i", channel)
            pass
        pass
    ...
    def get_rssi(self, channel):
        ...
        if p.returncode != 0:
            rospy.loginfo("[WiFi Det] get_rssi command FAILED")
            return
    ...
    def publish(self, time, id_, rssi):
        ...
        rospy.loginfo("[WiFi Det] %s.%s %s %i", msg.header.stamp.secs, msg.header.stamp.nsecs, msg.id, msg.strength)
