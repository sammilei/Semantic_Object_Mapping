## <div align="center">Guide on how to train yolov5 model for SubT</div>

First download data by running 
``` bash
download_data.sh
```

if you dont have permission to run the script, run `$ chmod +x download_data.sh`

next, if you would like to train a non-pruned model go to step 1, else go to step 2.
1. Train from scrath: 
    ```bash
    $ python train.py --img-size 1280 --batch 8 --epochs 60 --data SubT.yaml --weights yolov5m6.pt --multi-scale --hyp data/hyps/hyp.scrath.yaml --label-smoothing 0.1
    ```
    Pruning can then be done by running
    ```bash
    $ python prune.py --weights 'path-to-weights' --save-namne pruned.pt --prune-amount 0.1
    ```
    prune amount can be varied based on desired performance/speed, default is 0.1

2. Finetune the pruned model. A pruned model can be downloaded by running `git lfs fetch origin master`
    ```bash
    $ python train.py --img-size 1280 --batch 8 --epochs 60 --data SubT.yaml --weights pruned.pt --multi-scale --hyp data/hyps/hyp.prune.yaml --label-smoothing 0.1
    ```
3. Convert to TensorRT -> go [here](https://gitlab.robotics.caltech.edu/fets/artifact_detection_nn_torch2trt)
4. Recommended thresholds for the tested dataset will be exported to the test folder `runs/test/`
