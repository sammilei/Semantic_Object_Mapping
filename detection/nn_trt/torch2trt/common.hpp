...
int* loadShapes(std::string file) {
    ...
    size_t start_pos = file.find("wts");
    file.replace(start_pos, 3, "shapes");

    std::cout << "loading shapes from " << file << std::endl;

    std::ifstream input(file);
    assert(input.is_open() && "Unable to load shape file. please check if the .wts file path is right!!!!!!");

    // Read number of weight blobs
    int count;
    input >> count;
    assert(count > 0 && "Invalid weight map file.");
    static int shapes[30];
    for (int i = 0; i < count; i++)
    {
        input >> shapes[i];
        std::cout << "shape: " << i << ": " << shapes[i] << std::endl; 
...
IPluginV2Layer* addYoLoLayer_onnx(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, std::string lname, std::vector<ITensor*> dets) {
    auto creator = getPluginRegistry()->getPluginCreator("YoloLayer_TRT", "1");
    auto anchors = getAnchors(weightMap, lname);
    PluginField plugin_fields[2];
    int netinfo[4] = {Yolo::CLASS_NUM, Yolo::INPUT_W, Yolo::INPUT_H, Yolo::MAX_OUTPUT_BBOX_COUNT};
