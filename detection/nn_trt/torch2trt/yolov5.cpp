...
ICudaEngine* build_engine_p6_onnx(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt, float& gd, float& gw, std::string& wts_name) {
    const auto explicitBatch = 1U << static_cast<uint32_t>(nvinfer1::NetworkDefinitionCreationFlag::kEXPLICIT_BATCH);

    INetworkDefinition* network = builder->createNetworkV2(explicitBatch);
    std::map<std::string, Weights> weightMap = loadWeights("weights.wts");

    auto parser = nvonnxparser::createParser(*network, gLogger);

    parser->parseFromFile(const_cast<char*>(wts_name.c_str()),static_cast<int>(Logger::Severity::kWARNING));

    for (int i = 0; i < parser->getNbErrors(); ++i) {
        std::cout << parser->getError(i)->desc() << std::endl;
    }
    std::cout << network->getNbLayers() << " layers" << std::endl;
    std::cout << network->getNbOutputs() << " outputs" << std::endl;

    ITensor* det0 = network->getOutput(0);
    ITensor* det1 = network->getOutput(1);
    ITensor* det2 = network->getOutput(2);
    ITensor* det3 = network->getOutput(3);

    network->unmarkOutput(*det0);
    network->unmarkOutput(*det1);
    network->unmarkOutput(*det2);
    network->unmarkOutput(*det3);


    // network->markOutput(*network->getLayer(network->getNbLayers()-1)->getOutput(0));
    auto yolo = addYoLoLayer_onnx(network, weightMap, "model.33", std::vector<ITensor*>{det0, det1, det2, det3});
    yolo->getOutput(0)->setName(OUTPUT_BLOB_NAME);
    network->markOutput(*yolo->getOutput(0));
...
int main(int argc, char** argv) {
    ...
    for (int f = 0; f < (int)file_names.size(); f++) {
        ...
        for (int b = 0; b < fcount; b++) {
            ...
            cv::imwrite("output/_" + file_names[f - fcount + 1 + b], img);
        }
        fcount = 0;
    }
