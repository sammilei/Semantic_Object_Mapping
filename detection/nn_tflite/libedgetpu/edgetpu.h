...
public:
  ...
  // type.
  virtual std::unique_ptr<EdgeTpuContext>
  NewEdgeTpuContext(DeviceType device_type) = 0;

  // Same as above, but the created context is associated with the specified
  // type and device path.
  virtual std::unique_ptr<EdgeTpuContext>
  NewEdgeTpuContext(DeviceType device_type, const std::string& device_path) = 0;
  ...
  virtual std::unique_ptr<EdgeTpuContext>
  NewEdgeTpuContext(DeviceType device_type,
  ...
  // type.
  virtual std::shared_ptr<EdgeTpuContext>
  OpenDevice(DeviceType device_type) = 0;

  // Same as above, but the returned context is associated with the specified
  // type and device path. If path is empty, any device of the specified type
  // could be returned.
  virtual std::shared_ptr<EdgeTpuContext>
  OpenDevice(DeviceType device_type, const std::string& device_path) = 0;
  ...
  //  - "Usb.MaxBulkInQueueLength": ["0",.., "255"] (Default is "32")
  virtual std::shared_ptr<EdgeTpuContext>
  OpenDevice(DeviceType device_type,
             const std::string& device_path,
             const DeviceOptions& options) = 0;
  ...
  virtual std::vector<std::shared_ptr<EdgeTpuContext>>
  GetOpenedDevices() const = 0;
...
public:
  ...
  // if available.
  virtual const EdgeTpuManager::DeviceEnumerationRecord&
  GetDeviceEnumRecord() const = 0;
