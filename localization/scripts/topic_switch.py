...
class topic_switcher:
  ...
  def __init__(self):
    ...
    self.vel_sub = rospy.Subscriber(self.robot_namespace + "velodyne_points", PointCloud2, self.vel_clbk)
    ...
    self.vel_redir = rospy.Publisher(self.robot_namespace + "velodyne_points_redir", PointCloud2, queue_size = 100)
    self.vel_drain = rospy.Publisher(self.robot_namespace + "velodyne_points_drain", PointCloud2, queue_size = 100)
  ...
  def vel_clbk(self, msg):
    if rospy.get_param("topic_switch/velodyne") == True:
      self.vel_redir.publish(msg)
    else: 
      self.vel_drain.publish(msg)
