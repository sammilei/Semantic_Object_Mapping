 ...
 * - more concise and efficient, by grouping similar types of state variables
 * allowing for vectorized computations.
 */
...
Eigen::Quaterniond rotationVectorToQuaternion(const Eigen::Vector3d& r) {
  ...
  Eigen::Vector3d d_q_xyz =
      r * (d_q_w + Params::MRP_A) * (double(1.0) / Params::MRP_F);
...
// inline Eigen::Quaterniond quaternionDelta(const Eigen::Quaterniond & q, const
...
/** auto compute derivatives of all quaternions, given that ang. vel.s are
 * stored at vecs3d[omega_start:] */
...
void diffPosition(VecType& in_out, int p_start, int p_end, int v_start) {
  in_out.segment(p_start, p_end - p_start) =
      in_out.segment(v_start, p_end - p_start);
...
class RK4 {
public:
  template <class UKFModel, class InputType>
  static typename UKFModel::StateVec
  integrate(double delta,
            const typename UKFModel::StateVec& state,
            const InputType& u,
            const UKFModel& ukfModel) {
    typename UKFModel::StateVec a = ukfModel.dF(state, u);
    typename UKFModel::StateVec b =
        ukfModel.dF(typename UKFModel::StateVec(state + 0.5 * delta * a), u);
    typename UKFModel::StateVec c =
        ukfModel.dF(typename UKFModel::StateVec(state + 0.5 * delta * b), u);
    typename UKFModel::StateVec d =
        ukfModel.dF(typename UKFModel::StateVec(state + delta * c), u);
...
public:
  ...
  integrate(double delta,
            ...
            const InputType& u,
            const UKFModel& ukfModel) {
    typename UKFModel::StateVec initial = ukfModel.dF(state, u);
    typename UKFModel::StateVec predictor = state + delta * initial;
    return state + (delta * 0.5) * (initial + ukfModel.dF(predictor, u));
  }
};
...
public:
  ...
  integrate(double delta,
            ...
            const InputType& u,
            const UKFModel& ukfModel) {
    return state + delta * ukfModel.dF(state, u);
  }
};
} // namespace integrator
} // namespace util
...
class Vector
  : public Eigen::
        Matrix<double, NUM_SCALARS + NUM_3D_VECS * 3 + NUM_QUATERNIONS * 4, 1> {
public:
  typedef Eigen::
      Matrix<double, NUM_SCALARS + NUM_3D_VECS * 3 + NUM_QUATERNIONS * 4, 1>
          Base;
  ...
  static const int COV_SIZE =
      NUM_QUATERNIONS * 3 + NUM_3D_VECS * 3 + NUM_SCALARS;
  /** num sigma points */
  static const int NUM_SIGMA_POINTS = COV_SIZE * 2 + 1;

  /** type shorthands */
  // delta: for updates, which should be COV_SIZE in length (3 parameters only
  // for each quaternion)
  ...
  /* get a quaternion parameter at the given index (quat.col(idx), but as a
   * quaternion) */
  ...
  const Eigen::Map<const Eigen::Quaterniond> quat(int idx) const {
    Eigen::Map<const Eigen::Quaterniond> quatMap(this->data() + QUAT_START +
                                                 4 * idx);
    return quatMap;
  }
  ...
  void applyDelta(const VectorDelta& delta) {
    ...
    for (int i = 0; i < NUM_QUATERNIONS; ++i) {
      quat(i) = kalman::util::rotationVectorToQuaternion<Params>(
                    delta.template segment<3>(QUAT_START + 3 * i)) *
          quat(i);
    }
  }
...
Eigen::MatrixXd transform(VecType& mean, const Eigen::MatrixXd& S) {
  ...
  auto covs =
      S.template topLeftCorner<VecType::QUAT_START, VecType::COV_SIZE>();
  X.template topLeftCorner<VecType::QUAT_START, 1>() = vecs;
  X.template block<VecType::QUAT_START, VecType::COV_SIZE>(0, 1) =
      covs.colwise() + vecs;
  X.template block<VecType::QUAT_START, VecType::COV_SIZE>(
      0, VecType::COV_SIZE + 1) = -(covs.colwise() - vecs);

  for (int i = 0; i < VecType::_NUM_QUATERNIONS; ++i) {
    auto out = X.template block<4, VecType::NUM_SIGMA_POINTS>(
        VecType::QUAT_START + 4 * i, 0);
    auto state = mean.quat(i);
    auto cov =
        S.template block<3, VecType::COV_SIZE>(VecType::QUAT_START + 3 * i, 0);
    out.col(0) << state.vec(), state.w();

    Eigen::Array<double, 1, VecType::COV_SIZE> x_2 =
        cov.colwise().squaredNorm();
         ...
         Params::MRP_F *
             (x_2 * (1.0 - Params::MRP_A * Params::MRP_A) +
              Params::MRP_F * Params::MRP_F)
                 .sqrt()) /
        (Params::MRP_F * Params::MRP_F + x_2);
    Eigen::Array<double, 3, VecType::COV_SIZE> err_xyz =
        cov.array().rowwise() * (err_w + Params::MRP_A) * (1.0 / Params::MRP_F);

    Eigen::Quaterniond temp_q;
    for (std::size_t i = 0; i < VecType::COV_SIZE; i++) {
      temp_q = Eigen::Quaterniond(
                   err_w(i), err_xyz(0, i), err_xyz(1, i), err_xyz(2, i)) *
          state;
...
template <class UKFModel, class InputType>
Eigen::MatrixXd applyMeasurement(const Eigen::MatrixXd& X,
                                 const InputType& u,
                                 UKFModel& ukfModel) {
  Eigen::MatrixXd Xt(UKFModel::MeasureVec::SIZE,
                     UKFModel::StateVec::NUM_SIGMA_POINTS);
  for (int i = 0; i < UKFModel::StateVec::NUM_SIGMA_POINTS; ++i) {
    Xt.col(i) << ukfModel.H(X.col(i), u);
  }
  return Xt;
}

/** apply a continuous-time process, provided as a derivative class (with static
 * method 'dF'), to the Gaussian, where delta_t is the length of the time step.
...
void applyProcessInPlace(Eigen::MatrixXd& X,
                         const InputType& u,
                         double delta_t,
                         UKFModel& ukfModel) {
  for (int i = 0; i < UKFModel::StateVec::NUM_SIGMA_POINTS; ++i) {
    X.col(i) = ukfModel.G(X.col(i), u);
    X.col(i) = Integrator::template integrate<UKFModel, InputType>(
        delta_t, typename UKFModel::StateVec(X.col(i)), u, ukfModel);
  }
}
...
template <class UKFModel,
          class InputType,
          class Integrator = kalman::util::integrator::RK4>
Eigen::MatrixXd applyProcess(const Eigen::MatrixXd& X,
                             const InputType& u,
                             double delta_t,
                             const UKFModel& ukfModel) {
  Eigen::MatrixXd Xt = X;
  applyProcessInPlace<UKFModel, InputType, Integrator>(
      Xt, u, delta_t, ukfModel);
  return Xt;
}
...
SigmaVecType recoverMean(const Eigen::MatrixXd& X) {
  ...
  SigmaVecType mean = Params::sigma_WMI *
          X.template rightCols<StateVecType::NUM_SIGMA_POINTS - 1>()
              .rowwise()
              .sum() +
...
Eigen::MatrixXd computeDeltas(Eigen::MatrixXd& X, SigmaVecType& init_mean) {
  // recover
  Eigen::MatrixXd wPrime(SigmaVecType::COV_SIZE,
                         StateVecType::NUM_SIGMA_POINTS);
  wPrime.template topRows<SigmaVecType::QUAT_START>() =
      X.template topRows<SigmaVecType::QUAT_START>().colwise() -
  ...
  for (int i = 0; i < StateVecType::NUM_SIGMA_POINTS; ++i) {
    for (int j = 0; j < SigmaVecType::_NUM_QUATERNIONS; ++j) {
      int row_pos_in = SigmaVecType::QUAT_START + 4 * j,
          row_pos_out = SigmaVecType::QUAT_START + 3 * j;
      Eigen::Quaterniond delta_q =
          Eigen::Quaterniond(X.template block<4, 1>(row_pos_in, i)) *
          init_mean.quat(j).conjugate();
      wPrime.template block<3, 1>(row_pos_out, i) = Params::MRP_F *
          delta_q.vec() /
          (std::abs(Params::MRP_A + delta_q.w()) >
                   std::numeric_limits<double>::epsilon() ?
               Params::MRP_A + delta_q.w() :
               std::numeric_limits<double>::epsilon());
    }
  }
  return wPrime;
...
/** Inverse unscented transform - recover sqrt covariance. Note: input here is
 * deltas. Note 2: this is not used at all by the SRUKF. */
template <class Params, class VecType>
Eigen::MatrixXd recoverS(Eigen::MatrixXd& wPrime) {
  ...
  for (int i = 1; i < VecType::NUM_SIGMA_POINTS; i++) {
    cov.noalias() +=
        Params::sigma_WCI * (wPrime.col(i) * wPrime.col(i).transpose());
  }
  cov.noalias() +=
      Params::sigma_WC0 * (wPrime.col(0) * wPrime.col(0).transpose());
  return cov;
}
}; // namespace unscented
...
struct DefaultUKFParams {
  ...
  static constexpr double lambda =
      alphaSquared * (UKFModel::StateVec::COV_SIZE + 2.0) -
  ...
  */
  static constexpr double sigma_WM0 =
      lambda / (UKFModel::StateVec::COV_SIZE + lambda);
  static constexpr double sigma_WC0 = sigma_WM0 + (1.0 - alphaSquared + beta);
  static constexpr double sigma_WMI =
      1.0 / (2.0 * (UKFModel::StateVec::COV_SIZE + lambda));
  static constexpr double sigma_WCI = sigma_WMI;
};

// required namespace-scope declarations to avoid linker error
template <class UKFModel>
constexpr double DefaultUKFParams<UKFModel>::alphaSquared;
template <class UKFModel>
constexpr double DefaultUKFParams<UKFModel>::beta;
template <class UKFModel>
constexpr double DefaultUKFParams<UKFModel>::lambda;
template <class UKFModel>
constexpr double DefaultUKFParams<UKFModel>::MRP_A;
template <class UKFModel>
constexpr double DefaultUKFParams<UKFModel>::MRP_F;
template <class UKFModel>
constexpr double DefaultUKFParams<UKFModel>::sigma_WM0;
template <class UKFModel>
constexpr double DefaultUKFParams<UKFModel>::sigma_WC0;
template <class UKFModel>
constexpr double DefaultUKFParams<UKFModel>::sigma_WMI;
template <class UKFModel>
constexpr double DefaultUKFParams<UKFModel>::sigma_WCI;
 ...
 *  >> member FUNCTIONS StateVec dF(StateVec) for finding derivative (process
 * model) MeasureVec H(StateVec) to compute measurement for state (measurement
 * model) void init(UKF) to initialize state, covariances etc. InputType: type
 * of input for process UKFParam: optionally, class with parameters. Integrator:
 * optionally, numerical integrator to use. Should be
 * ark::kalman::util::integrator::XYZ
 */
template <class UKFModel,
...
public:
  UKF(UKFModel ukf_model) {
    UKFModel::init(*this, ukf_model);
  }
  ...
  /** measurement noise root covariance (constant); please specify this in init
   * (should be diagonal) */
  ...
  /** SR-UKF update, adapted from github.com/sfwa/ukf, reference: Van der Merwe
   * 2001 */
  void update(double delta_t, const MeasureVec& z, const InputType& input) {
    // std::cout << objParams.rvec_.at(0) << std::endl ;

    // 'a priori' step
    /* calc sigma points */
    sigmaPoints = unscented::template transform<UKFParams, StateVec>(
        state,
        Eigen::MatrixXd(stateRootCov *
                        std::sqrt(StateVec::COV_SIZE + UKFParams::lambda)));

    /* Propagate the sigma points through the process model. */
    unscented::applyProcessInPlace<UKFModel, InputType, Integrator>(
        sigmaPoints, input, delta_t, ukfModel);
    ...
    */
    Eigen::MatrixXd wPrime_RC(StateVec::COV_SIZE,
                              StateVec::NUM_SIGMA_POINTS - 1 +
                                  StateVec::COV_SIZE);
    wPrime_RC << std::sqrt(UKFParams::sigma_WCI) *
            wPrime.rightCols(StateVec::NUM_SIGMA_POINTS - 1),
        processNoiseRootCov;

    /*
    Calculate the QR decomposition of the augmented sigma point deltas.
    */
    stateRootCov = wPrime_RC.transpose()
                       .householderQr()
                       .matrixQR()
                       .topLeftCorner(StateVec::COV_SIZE, StateVec::COV_SIZE)
    ...
    */
    ...
    // 'innovation' step
    Eigen::MatrixXd measuredSigmaPoints =
        unscented::template applyMeasurement<UKFModel, InputType>(
            sigmaPoints, input, ukfModel);

    MeasureVec zPred = unscented::recoverMean<UKFParams, StateVec, MeasureVec>(
        measuredSigmaPoints);
    zPrime = unscented::computeDeltas<UKFParams, StateVec, MeasureVec>(
        measuredSigmaPoints, zPred);
    ...
    */
    Eigen::MatrixXd zPrime_RC(z.size(),
                              StateVec::NUM_SIGMA_POINTS - 1 + z.size());
    zPrime_RC.template block<MeasureVec::SIZE, StateVec::NUM_SIGMA_POINTS - 1>(
        0, 0) = std::sqrt(UKFParams::sigma_WCI) *
        zPrime.rightCols(StateVec::NUM_SIGMA_POINTS - 1);
    zPrime_RC.template block<MeasureVec::SIZE, MeasureVec::SIZE>(
        0, StateVec::NUM_SIGMA_POINTS - 1) = measureNoiseRootCov;
    ...
    innovationRootCov =
        zPrime_RC.transpose()
            .householderQr()
            .matrixQR()
            .template topLeftCorner<MeasureVec::SIZE, MeasureVec::SIZE>()
            .template triangularView<Eigen::Upper>();
    ...
    */
    Eigen::MatrixXd crossCorr =
        Eigen::MatrixXd::Zero(StateVec::COV_SIZE, MeasureVec::SIZE);
    for (std::size_t i = 1; i < StateVec::NUM_SIGMA_POINTS; i++) {
      crossCorr.noalias() +=
          UKFParams::sigma_WCI * (wPrime.col(i) * zPrime.col(i).transpose());
    }
    crossCorr.noalias() +=
        UKFParams::sigma_WC0 * (wPrime.col(0) * zPrime.col(0).transpose());
    ...
    */
    Eigen::MatrixXd kalmanGain =
        innovationRootCov.transpose()
            .fullPivHouseholderQr()
            .solve(innovationRootCov.fullPivHouseholderQr().solve(
                crossCorr.transpose()))
            .transpose();
  ...
  void defaultInitialize(double alphaStateRootCov = 1e-3,
    ...
    stateRootCov =
        Eigen::MatrixXd::Identity(StateVec::COV_SIZE, StateVec::COV_SIZE) *
        alphaStateRootCov;
    processNoiseRootCov =
        Eigen::MatrixXd::Identity(StateVec::COV_SIZE, StateVec::COV_SIZE) *
        alphaProcRootCov;
    measureNoiseRootCov =
        Eigen::MatrixXd::Identity(MeasureVec::COV_SIZE, MeasureVec::COV_SIZE) *
        alphaMeasRootCov;
  }
};
} // namespace kalman
